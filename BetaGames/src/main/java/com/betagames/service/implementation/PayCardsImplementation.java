...
public class PayCardsImplementation implements IPayCardsService {
    ...
    public void create(PayCardsRequest req) throws Exception {
        ...
        if (paycard.isPresent()) throw new Exception("Pay Card already present for this user!");
    ...
    public void update(PayCardsRequest req) throws Exception {
        ...
        if (!updtPayCard.getCardNumber().equals(req.getCardNumber())) {
            // Se il numero è stato modificato, verifico che il nuovo numero non sia già in uso da un'altra carta dello stesso utente
            Optional<PayCards> existingCard = paycardsR.findByCardNumberAndUserId(req.getCardNumber(), updtPayCard.getUser().getId()); //ID utente PRESO DALLA CARTA
            if (existingCard.isPresent() && !existingCard.get().getId().equals(req.getId())) {
                throw new Exception("This user already has a card with this number!");
            }
        ...
        //updtPayCard.setUser(user.get()); // CAPIRE SE SERVE ANDARE A MODIFICARE L'USER... HA SENSO? ------> NO
        ...
        updtPayCard.setUpdatedAt(now); // impostarlo in automatico all'update (al momento della creazione sarà uguale alla createdAt)
